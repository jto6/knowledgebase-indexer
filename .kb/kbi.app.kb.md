...
> `kbi.py` is the main orchestrator — it wires together file discovery, handler dispatch, index model construction (FS/keyword/tag/word/deps/glossary views), rendering, and a token-efficient `--update-cards` pipeline (staleness → delta → `/kb-card` handoff → auto-commit → audit trail), all driven by `KnowledgebaseIndexer`.
...
- **`--update-cards` mode** (two-level staleness check, then a per-directory content delta handed to `/kb-card`):
...
- **`main()` dispatch**: `search`/`hash`/`manifest-sync`/`decisions` as `argv[0]` route to their `run_*` function before the main argparse parser runs; otherwise the normal config-driven index-generation path (with `--update-cards`/`--no-commit`) applies
