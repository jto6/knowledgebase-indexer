...
- **Exported symbols**: `__all__ = ['KnowledgebaseIndexer', 'run_search', 'run_hash', 'run_manifest_sync', 'run_decisions', '_compute_dir_hash']` — the public test-facing surface, extended for the `kbi hash`/`manifest-sync`/`decisions` helper subcommands and the shared `dir_hash` formula
